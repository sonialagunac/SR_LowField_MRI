...
This repository has been adapted by Sonia Laguna, to focus on the ESRGAN implementation, a network for image super-resolution. It has been modified to be used with nifti files and 3D data, as well as with specific data augmentation techniques in the project. It is the backbone of the work presented on [Super-resolution of portable low-field MRI in real scenarios: integration with denoising and domain adaptation](https://openreview.net/forum?id=pinw0Gcot4T). 

As a summary, this part of the project focuses on the super-resolution of low-field MRI data to high-resolution MRI data. 
This repository includes the architectures of the denoiser module and the domain adaptation modules described in the [report]((https://openreview.net/forum?id=pinw0Gcot4T)). 

The Super-Resolution model alone is created in this implementation, aswell as the concatenation and finetuning of the super-resolution with the denoiser, and the super-resolution with the denoiser and domain adapataion. 
Note that the perceptual loss from VGG used in the regular ESRGAN methods has been removed since we are now working on 3D data. To differentiate the different steps of the meta-architecture training procedure, the parameters in the option .yaml files ([train](./options/train/ESRGAN/train_ESRGAN_x4_sonia.yml) and [test](./options/test/ESRGAN/test_ESRGAN_x4_woGT_sonia.yml)) have been modified. 

Relevant parameters to be modified for the MRI Super-Resolution project:
  - `name`: Name of the directory where the checkpoints or results will be stored
  - `datasets/(test_1/train)/cycle`: True when working with the final finetuning of the meta-architecture with the CycleGAN as domain adaptation, False else. 
  - `datasets/(test_1/train)/cut`: True when working with the final finetuning of the meta-architecture with the FastCUT as domain adaptation, False else.
  - `datasets/(test_1/train)/denois`: True when working with concatenating the denoiser and super resolution modules and training them together, False else. 
  - Specific to training .yaml:
      - `datasets/train/dataroot_gt`: Directory with high resolution HCP data, will be degraded to the corresponding low-resolution pair
      - `datasets/train/dataroot_lq`: Directory used solely for the final meta-architecture finetuning. Corresponds to the low-field MRI data.
      -  `datasets/train/dataroot_seg`: Directory with segmentations of the high-resolution HCP data. Used for segmentation based intensity mask in data augmentation. 
      -  `datasets/train/model_sr`: Name of the pretrained model of super-resolution to load when concatenating with the denoiser. Only used when `denois`is True.
      -  `datasets/train/epoch_sr`: Epoch corresponding to model_sr
      -  `datasets/train/model_d`: Name of the pretrained model of denoising to load when concatenating super-resolution with the denoiser. Only used when `denois`is True.
      -  `datasets/train/epoch_d`: Epoch corresponding to model_d
      -  `datasets/train/model_srd`: Name of the pretrained model of super-resolution and denoising concatenated,  to load when concatenating domain adapatation with super-resolution and the denoiser. Only used when `cycle` or `cut` are True.
      -  `datasets/train/epoch_srd`: Epoch corresponding to model_srd
      -  `datasets/train/model_daf`: Name of the pretrained model of FastCUT domain adaptation, to load when concatenating domain adaptation with super-resolution and the denoiser. Only used when `cut` is True.
      -  `datasets/train/epoch_daf`: Epoch corresponding to model_daf
      -  `datasets/train/model_da`: Name of the pretrained model of CycleGAN domain adaptation, to load when concatenating domain adaptation with super-resolution and the denoiser. Only used when `cycle` is True.
      -  `datasets/train/epoch_da`: Epoch corresponding to model_da
  - Specific to testing .yaml:
      - `datasets/test_1/dataroot_lq`: Directory with data used for inference. Corresponds to the low-field MRI test data.
      - `path/pretrain_network_g`: Path to the checkpoint to load for inference. Very important that it corresponds to the same architecture as defined in the `cycle` or `cut` parameters. 
  
To run this implementation the following commands can be used: 
 - Training: `python ./basicsr/train.py -opt ./options/train/ESRGAN/name_of_train_config.yml`
` 
- Inference: `python ./basicsr/test.py -opt ./options/test/ESRGAN/name_of_test_config.yml`


---
The original README file and extended information on the BasicSR repository is found below: 
