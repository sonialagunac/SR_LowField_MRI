...
class SRModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        net_g = self.model_to_device(net_g)
        # Loading the denoiser architecture
        if 'train' in self.opt['datasets']: # If we are training
            if self.opt['datasets']['train']['cut'] == 0 and self.opt['datasets']['train']['cycle'] ==0 and self.opt['datasets']['train']['denois'] == 1:
                pretrained = DnCNN().to(torch.cuda.current_device())
                # Loading previous SR weights
                resume_state_path_sr = os.path.join('/autofs/cluster/HyperfineSR/sonia/BasicSR-master_3D/experiments/',
            ...
            if self.opt['datasets']['test_1']['cut'] == 0 and self.opt['datasets']['test_1']['cycle'] ==0 and self.opt['datasets']['test_1']['denois'] == 1:
                pretrained = DnCNN().to(torch.cuda.current_device())
                self.net_g = combo(net_g, pretrained)
        ...
        if 'train' in self.opt['datasets']:  # If we are training
            if self.opt['datasets']['train']['cut'] == 1 or self.opt['datasets']['train']['cycle'] == 1:
                ...
                resume_state_path_da = os.path.join('/autofs/cluster/HyperfineSR/sonia/BasicSR-master_3D/experiments/',
            ...
            if self.opt['datasets']['train']['cycle']:
                ...
                self.net_g = combo_GAN(trained, net).to(torch.cuda.current_device())

            if self.opt['datasets']['train']['cut']:
                net = ResnetGenerator(1, 1, 64, norm_layer=functools.partial(nn.BatchNorm3d, affine=True,track_running_stats=True),
    ...
    def feed_data(self, data):
        ...
        if 'lq_hcp' in data:
            self.hcp= data['lq_hcp'].to(self.device)
    ...
    def test(self):
        # print(torch.max(self.lq), torch.min(self.lq), 'max min lq')
